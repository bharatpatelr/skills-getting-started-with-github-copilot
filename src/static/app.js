document.addEventListener("DOMContentLoaded", () => {
  ...
  // Helper: simple HTML escape to avoid inserting raw user input
  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Build participants section (bulleted list or friendly message)
               ...
               <ul class="participants-list">
                 ${details.participants.map((p) => `
                   <li class="participant-item">
                     ...
                     <button class="delete-icon" onclick="unregisterParticipant('${escapeHtml(name)}', '${escapeHtml(p)}')" title="Remove participant">×</button>
                   </li>
                 `).join("")}
               </ul>`
